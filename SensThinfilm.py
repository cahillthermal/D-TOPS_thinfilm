...
def SensThinfilm(
    ...
    """
    Sensitivity calculation of thin film D-TOPS model.
    """
    f = np.atleast_1d(f)
    ...
    sqrtC11rho_1 = np.sqrt((1 + (1e-6) * 1j) * C11_1 / rho_1)
    ...
    sqrtC11rho_2 = np.sqrt((1 + (1e-4) * 1j) * C11_0_2 / rho_2)
    ...
    sqrtC11rho_3 = np.sqrt((1 + (1e-4) * 1j) * C11_0_3 / rho_3)
    ...
    n_f = len(f)
    theta_deformation = np.empty((n_f, 1), dtype=complex)
    theta_dndT_air = np.empty((n_f, 1), dtype=complex)
    theta = np.empty((n_f, 1), dtype=complex)

    V_signal_deformation = np.empty((n_f, 1), dtype=complex)
    V_signal_dndT_air = np.empty((n_f, 1), dtype=complex)
    V_signal = np.empty((n_f, 1), dtype=complex)
    ...
    for i_fr in range(n_f):
        ...
        for i_p in range(n_p):
            ...
            sum2_2_b = Q_2[3:6, :] @ t2
            BCC[6:9] = -sum1_b + c11_ratio_21 * sum2_2_b if 'sum1_b' in locals() else -sum1_1_b + c11_ratio_21 * sum2_2_b
            ...
            Z_deformation_p_omega = -(w_s_H + w_s_P)
            Z_dndT_p_omega = -dndT_4 * thetas_1 / zeta_4

            # integrand for calculating PBD
            be = bes_exp[i_p]
            I_p2[i_p] = Z_deformation_p_omega * be
            I_air_p2[i_p] = Z_dndT_p_omega * be

        theta_deformation[i_fr, 0] = c_p_pi * np.dot(simp_weights, I_p2)
        theta_dndT_air[i_fr, 0] = c_p_pi * np.dot(simp_weights, I_air_p2)

    # sum up PBD angle sources
    if flag_vacuum > 0:
        theta[:, 0] = theta_deformation[:, 0] + theta_dndT_air[:, 0]
    else:
        theta[:, 0] = theta_deformation[:, 0]

    # conversion from PBD angle to signal of lock-in amplifier
    V_signal_deformation[:, 0] = (
        det_factor * theta_deformation[:, 0] * V_SUM / np.sqrt(2.0)
    )
    V_signal_dndT_air[:, 0] = (
        det_factor * theta_dndT_air[:, 0] * V_SUM / np.sqrt(2.0)
    )
    V_signal[:, 0] = det_factor * theta[:, 0] * V_SUM / np.sqrt(2.0)
